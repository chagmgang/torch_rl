...
def train(batch_size, memory, target_network, main_network):
    ...
    Qmain_next_state = torch.argmax(main_network(tensor_next_state), dim=1).data.numpy()
    one_hot_next_action = Variable(torch.from_numpy(np.array([np.eye(2)[ac] for ac in Qmain_next_state])).float())
